    ...
    userDao.prototype.select = function (param, callback) {
        this.pool.getConnection(function (err, conn) {
            ...
            conn.query(sql, arr, function (err, ret) {
                if (ret) {
                    result = new db.result('200', '成功', ret);
                    callback(result);
                }
                else {
                    result = new db.result('200', '失败', err);
                    callback(result);
                }
                // 释放连接
                conn.release();
...
para.tableName = 'user_info';
para.whereField = [{ key: 'userId', value: '4' }, { key: 'userName', value: '张一' }];
userdao.select(para, function (obj) { console.log(obj); });

...
(function (Intent_1) {
    var IntentMgr = (function () {
        ...
        IntentMgr.prototype.executeIntent = function (IntentObject, executeStepName, callback) {
            ...
            if ("enter" == executeStepName) {
                ...
                if (IntentObject.entities.hasOwnProperty(executeStepName)) {
                    ...
                    var actionReq_ = { AIAgentData: IntentObject.AIagentData, entity: IntentObject.entities };
        ...
        //执行LUIS过来的意图
        IntentMgr.prototype.ExecuteLUISIntent = function (intent, _AIagentData, intentObject, callback) {
            var _this = this;
        ...
        //执行手动创建的意图
        IntentMgr.prototype.ExecuteIntent = function (intent, callback) {
            this.executeIntent(intent, "enter", callback);
        };
        //将用户意图存入数据库
        IntentMgr.prototype.SaveUserIntentToRedis = function (IntentObject) {
            var IntentJson = {};
        ...
        //将AI意图存入数据库
        IntentMgr.prototype.SaveAIIntentToRedis = function (IntentObject) {
            var IntentJson = {};
            for (var key in IntentObject.entities) {
                if (IntentObject.entities[key].entity.length > 0) {
                    IntentJson[IntentObject.entities[key].name] = IntentObject.entities[key].entity;
            ...
            redis.GetItemFromHash(_AIagentData.familyID, LUISIntentObject.name, function (err, res) {
                if (err) {
                    ...
                    if (res == null) {
                        ...
                        Intent = _this.IntentJosnStringToObject(LUISIntentObject.name, res, _AIagentData);
                        callback(Intent);
                    }
                    redis.Quit();
                }
        ...
        IntentMgr.prototype.IntentJosnStringToObject = function (IntentName, IntentJosnString, _AIagentData) {
            var IntentJosnObject = JSON.parse(IntentJosnString);
            var Intent = new this.IntentDict[IntentName]();
            Intent.AIagentData = _AIagentData;
            for (var key in Intent.entities) {
